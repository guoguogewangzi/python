...
def detect(content, **kwargs):
    ...
    server = headers.get(HTTP_HEADER.HTTP_HEADER.SERVER, "")
    cookie = headers.get(HTTP_HEADER.HTTP_HEADER.COOKIE, "")
    set_cookie = headers.get(HTTP_HEADER.HTTP_HEADER.SET_COOKIE, "")
    cf_ray = headers.get(HTTP_HEADER.HTTP_HEADER.CF_RAY, "")
    if cf_ray != "":
        return True
    expect_ct = headers.get(HTTP_HEADER.HTTP_HEADER.EXPECT_CT, "")
