...
#内置库
import asyncio,random,json,sys,hashlib,time,itertools,random
...
def dump(urllist, proxies, header_new):
    def download(url: str, fname: str, proxies: str, newheader):
        ...
        total = int(resp.header.get('content-length',0))
    ...
    try:
        if str(requests.head(url1,verify=False)) != "<Response [200]>":
            ...
            sys.exit()
        if str(requests.head(url2,verify=False)) != "<Response [200]>":
            ...
            download(url, "heapdump", proxies, newheader)
            sys.exit()
        if str(requests.head(url3,verify=False)) != "<Response [200]>":
            ...
            sys.exit()
        if str(requests.head(url4,verify=False)) != "<Response [200]>":
            ...
            download(url, "heapdump", proxies, newheader)
            sys.exit()
        if str(requests.head(url5,verify=False)) != ("<Response [401]>" or "<Response [200]>"):
            ...
            sys.exit()
        if str(requests.head(url6,verify=False)) != "<Response [200]>":
