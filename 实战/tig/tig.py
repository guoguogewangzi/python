...
+-+-+-+-+-+-+ +-+-+-+-+-+-+-+-+-+-+-+-+ +-+-+-+-+-+-+-+-+-+
    ...
    #是否指定配置文件
    #################################################
    #指定了配置文件,则配置文件路径保存至:config_path
    if args.config:
    ...
    #没有指定配置文件,则自输入:ThreatBook_api,Fofa_email,Fofa_api
    else:
    ...
    #没有指定结果文件，则创建名为‘output’文件夹，结果文件路径保存至：tig_output
    else:
        root_path = sys.path[0]
